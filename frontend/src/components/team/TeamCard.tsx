import { clsx } from "clsx";
import { TeamWithRelations } from "../../types/Team";
import TeamActionMenu from "./TeamActionMenu";
import { useNavigate } from "react-router-dom";
import TeamNameAcronym from "./TeamNameAcronym";

/**
 * Prorpiéts du TeamCardProps
 *
 * - team : l'équipe dont on veut afficher la carte
 */
type TeamCardProps = {
  team: TeamWithRelations;
};

/**
 * Affiche la carte de base d'une équipe :
 *  - Nom de l'équipe, acronyme du nom et le bouton de menu
 *
 * @param {TeamCardProps} param0 - Proprités du TeamCard
 */
export default function TeamCard({ team }: TeamCardProps) {
  const navigate = useNavigate();
  return (
    <div
      className={clsx(
        "relative flex h-40 max-h-40 w-52 max-w-52 cursor-pointer flex-col px-4 py-4",
        "gap-5 rounded-md border border-gray-300 shadow-md hover:brightness-90",
        "dark:border-gray-600 dark:bg-gray-800"
      )}
    >
      <div className="absolute inset-0 bg-transparent" onClick={() => navigate(`${team.id}`)} />
      <div className={clsx("flex w-full justify-center")}>
        <TeamNameAcronym id={team.id} name={team.name} className="min-w-16 px-3 py-5" />
      </div>
      <div className={clsx("flex w-full items-start justify-between pt-2")}>
        <span className={clsx("max-w-36 text-left text-sm font-normal text-wrap text-gray-600")}>
          {team.name}
        </span>
        <div className="relative z-10">
          <TeamActionMenu team={team} />
        </div>
      </div>
    </div>
  );
}
