...
import { emailQueue, addEmailToQueue } from "./email.queue";
import { successResponse, errorResponse } from "../utils/apiResponse";
import { sendWelcomeEmailSchema } from "./email.schemas";
import { z } from "zod";
...
export const sendWelcomeEmail = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json(errorResponse("INTERNAL_SERVER_ERROR", "Erreur lors de l’envoi d’e-mail"));  
  }
};
