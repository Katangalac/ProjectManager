...
export const setupSocket = (server: http.Server) => {
  ...
  io.on("connection", async (socket) => {
    try {
      ...
      const conversationsCollection = await getUserConversations(
        socket.userId,
        {
          all: true,
          page: 1,
      ...
      conversationsCollection.conversations.forEach((conversation) => {
        socket.join(conversation.id);
        console.log(`User ${socket.userId} join one of his conversation`);
      });

      const teamsCollection = await getUserTeams(socket.userId, {
        all: true,
        page: 1,
        pageSize: 20,
      });

      teamsCollection.teams.forEach((team) => {
        socket.join(team.id);
