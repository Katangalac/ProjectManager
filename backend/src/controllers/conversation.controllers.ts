...
export const createConversationController = async (
  ...
  try {
    const conversationData = conversationSchema.createConversationSchema.parse(
      req.body,
    );
    const conversation =
...
export const getConversationByIdController = async (
  ...
  try {
    ...
    res
      .status(200)
      .json(successResponse(conversation, "Conversation récupérée"));
  } catch (err) {
    ...
    if (err instanceof ConversationNotFoundError) {
      res
        ...
        .json(
          errorResponse(
            err.code ? err.code : "CONVERSATION_NOT_FOUND",
            err.message,
          ),
        );
    }
...
export const updateConversationController = async (
  ...
  try {
    ...
    res
      .status(200)
      .json(successResponse(updatedConversation, "Conversation mise à jour"));
  } catch (err) {
 ...
 * @param {Response} res - reponse express en JSON
 */
export const getConversationsParticipantsController = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    res
      ...
      .json(
        successResponse(
          participants.users,
          "Participants de la conversation récupérés",
          participants.pagination,
        ),
      );
  } catch (err) {
    console.error(
      "Erreur lors de la récupération des participants de la conversation",
      err,
    );
    if (err instanceof z.ZodError) {
      res.status(400).json(errorResponse("INVALID_REQUEST", err.message));
...
export const getConversationMessagesController = async (
  ...
  try {
    ...
    const messagesCollection =
      await conversationService.getConversationMessages(id, filter);
    res
      .status(200)
      .json(
        successResponse(
          messagesCollection.messages,
          "Messages de la conversation récupérées",
          messagesCollection.pagination,
        ),
      );
  } catch (err) {
    console.error(
 ...
 * @param {Response} res - reponse express en JSON
 */
export const addParticipantToConversationController = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    res
      ...
      .json(
        successResponse(
          userConversationPair,
          "Utilisateur ajouté dans la conversation",
        ),
      );
  } catch (err) {
    console.error(
      "Erreur lors de l'ajout d'un participant dans la conversation",
      err,
    );
    if (err instanceof z.ZodError) {
      res.status(400).json(errorResponse("INVALID_REQUEST", err.message));
    ...
    if (err instanceof UserAlreadyInConversationError) {
      res
        ...
        .json(
          errorResponse(
            err.code ? err.code : "USER_ALREADY_IN_CONVERSATION",
            err.message,
          ),
        );
    }
