...
export const createNotificationController = async (
  ...
  try {
    const notificationData = notificationSchema.createNotificationSchema.parse(
      req.body,
    );
    ...
    res
      .status(201)
      .json(successResponse(newNotification, "Notification créée"));
  } catch (err) {
...
export const getNotificationsController = async (
  ...
  try {
    const filter = notificationSchema.searchNotificationsFilterSchema.parse(
      req.query,
    );
    ...
    res
      ...
      .json(
        successResponse(
          ...
          "Notifications récpérées",
          notificationsCollection.pagination,
        ),
      );
  } catch (err) {
...
export const getNotificationByIdController = async (
  ...
  try {
    ...
    res
      .status(200)
      .json(successResponse(notification, "Notification récupérée"));
  } catch (err) {
    ...
    if (err instanceof NotificationNotFoundError) {
      res
        ...
        .json(
          errorResponse(
            err.code ? err.code : "NOTIFICATION_NOT_FOUND",
            err.message,
          ),
        );
    }
...
export const markNotificationAsReadController = async (
  ...
  try {
    ...
    res
      .status(200)
      .json(successResponse(updatedNotification, "Notification marquée lue"));
  } catch (err) {
