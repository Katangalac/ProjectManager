...
export const getTaskByIdController = async (req: Request, res: Response) => {
  try {
    ...
    if (err instanceof taskError.TaskNotFoundError) {
      res
        .status(404)
        .json({ error: "Aucune tâche correspond à l'idetifiant donné" });
    }
    res
      .status(500)
 ...
 * @param {Response} res - reponse Express en JSON
 */
export const assignTaskToUserController = async (
  req: Request,
  res: Response,
) => {
  try {
 ...
 * @param {Response} res - reponse Express en JSON
 */
export const unassignUserFromTaskController = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    console.error(
      "Erreur lors de la désassignation de l'utilisateur de la tâche : ",
      err,
    );
    if (err instanceof z.ZodError) {
      res.status(400).json({ error: "Données invalides" });
 ...
 * @param {Response} res - reponse Express en JSON
 */
export const getTaskContributorsController = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la récupération des contributeurs" });
  }
};
